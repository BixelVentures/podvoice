...
SOURCE_SPEAKER_SCHEMA = speaker.SPEAKER_SCHEMA.extend(
    {
        ...
        cv.Optional(CONF_BUFFER_DURATION, default="100ms"): cv.positive_time_period_milliseconds,
    ...
    cv.Schema(
        {
            ...
            cv.Required(CONF_DECIBEL_REDUCTION): cv.templatable(cv.int_range(min=0, max=51)),
...
async def ducking_set_to_code(config, action_id, template_arg, args):
    ...
    decibel_reduction = await cg.templatable(config[CONF_DECIBEL_REDUCTION], args, cg.uint8)
