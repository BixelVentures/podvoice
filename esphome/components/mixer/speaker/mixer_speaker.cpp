  ...
  switch (this->state_) {
    ...
    case speaker::STATE_STOPPING: {
      if ((this->parent_->get_output_speaker()->get_pause_state()) ||
        ...
        this->podvoice_forced_stop_ = true;
...
esp_err_t SourceSpeaker::start_() {
  ...
  if (this->audio_source_.use_count() == 0) {
    ...
    this->podvoice_last_source_ = this->audio_source_;
    this->podvoice_forced_stop_ = false;
  }
...
bool SourceSpeaker::podvoice_quiescent() const {
  const uint32_t commands = SOURCE_SPEAKER_COMMAND_START | SOURCE_SPEAKER_COMMAND_STOP | SOURCE_SPEAKER_COMMAND_FINISH;
  return this->is_stopped() && this->event_group_ != nullptr &&
         !(xEventGroupGetBits(this->event_group_) & commands) &&
         this->podvoice_last_source_.expired() && !this->podvoice_forced_stop_;
}
