...
void MicroWakeWord::setup() {
  ...
  this->detection_queue_ = xQueueCreate(DETECTION_QUEUE_LENGTH, sizeof(QueuedDetection));
...
void MicroWakeWord::loop() {
  ...
  switch (this->state_) {
    case State::STARTING:
      ...
      break;
    case State::DETECTING_WAKE_WORD: {
      QueuedDetection queued;
      while (xQueueReceive(this->detection_queue_, &queued, 0)) {
        if (queued.stop_command != 0) {
          if (this->stop_gate_.accepts(queued.stop_command))
            this->stop_detected_callback_.call(queued.stop_command);
          continue;
        }
        const DetectionEvent &detection_event = queued.event;
...
void MicroWakeWord::process_probabilities_() {
  ...
  for (auto &model : this->wake_word_models_) {
    if (model->get_unprocessed_probability_status()) {
      ...
      if (model == this->stop_model_) {
        this->stop_gate_.observe(model->is_ready_for_detection(),
                                wake_word_state.max_probability < model->get_probability_cutoff(),
                                this->microphone_source_->get_audio_stream_info().get_sample_rate() * FEATURE_DURATION_MS / 1000);
        bool detected = wake_word_state.detected;
#ifdef USE_MICRO_WAKE_WORD_VAD
        detected = detected && vad_state.detected;
#endif
        uint32_t command = this->stop_gate_.detect(detected);
        if (command != 0) {
          QueuedDetection queued{wake_word_state, command};
          if (xQueueSend(this->detection_queue_, &queued, 0) != pdTRUE)
            this->stop_gate_.invalidate();
