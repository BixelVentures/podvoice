...
SetPlaylistDelayAction = speaker_source_ns.class_("SetPlaylistDelayAction", automation.Action)
        ...
        if pipeline_config := config.get(pipeline_key):
            ...
            if pipeline_config[CONF_FORMAT] != "NONE":
                cg.add(
                    var.set_format(
                        pipeline_enum,
                        media_player.build_supported_format_struct(pipeline_config, purpose),
                    )
                )
