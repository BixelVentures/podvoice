...
bool SpeakerSourceMediaPlayer::podvoice_announcement_idle() const {
  const auto &ps = this->pipelines_[ANNOUNCEMENT_PIPELINE];
  // No queued start may survive an idle snapshot before STOP was processed.
  if (this->media_control_command_queue_ == nullptr || uxQueueMessagesWaiting(this->media_control_command_queue_) != 0 ||
      ps.active_source != nullptr || ps.pending_source != nullptr || ps.stopping_source != nullptr ||
      ps.playlist_index < ps.playlist.size()) return false;
  for (const auto &binding : ps.sources) {
    if (binding->source->get_state() != media_source::MediaSourceState::IDLE) return false;
  }
  return true;
}
