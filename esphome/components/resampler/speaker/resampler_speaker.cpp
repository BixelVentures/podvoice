...
bool ResamplerSpeaker::podvoice_quiescent() const {
  const uint32_t commands = ResamplingEventGroupBits::COMMAND_START | ResamplingEventGroupBits::COMMAND_STOP |
                            ResamplingEventGroupBits::COMMAND_FINISH;
  return this->is_stopped() && !this->task_.is_created() && this->event_group_ != nullptr &&
