...
(async () => {
  ...
  try {
    for (const width of [320, 390, 1440]) {
      ...
      await select.selectOption('hey_chat_hey_jarvis');
      await page.locator('#s_save').click();
      await page.waitForFunction(() => document.querySelector('#s_wake_word_status').textContent.includes('Gemt valg (alle enheder): Hey Chat + Hey Jarvis'));
      assert.match(await page.locator('#s_wake_word_status').innerText(), /Bekræftet af enheden: Okay Nabu/);
      assert.equal(await page.locator('#wake-example').textContent(), '“Hey Chat, hvad er klokken?” eller “Hey Jarvis, hvad er klokken?”');
      ...
      assert.equal(saved.wake_word, 'hey_chat_hey_jarvis');
      ...
      await page.evaluate(() => { rooms.Stue = {connected:true, wake_word_supported:true, wake_word_confirmed:'hey_chat_hey_jarvis'}; renderWakeWordStatus(); });
      offline = true;
